...
export default function Programs() {
  const programs = [
    ...
    },
    {
      ...
      icon: "🌐",
      color: "from-accent/20 to-tertiary/20",
  ...
  return (
    <>
      <section className="py-20 md:py-32 bg-gradient-to-b from-white via-primary/5 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {programs.map((program) => (
            ...
            >
              {/* Background accent */}
              <div className="absolute top-0 right-0 w-20 h-20 bg-highlight/5 rounded-full -mr-8 -mt-8 group-hover:scale-150 transition-transform"></div>
              ...
              {/* Hover border effect */}
              <motion.div
                className="absolute inset-0 rounded-2xl border-2 border-accent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"
              ></motion.div>
            </motion.div>
          ))}
        </motion.div>
        ...
        >
          <Link href="/programs">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-8 py-4 bg-accent text-white rounded-lg font-bold text-lg hover:shadow-accent transition-all"
            >
